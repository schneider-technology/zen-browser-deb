...
ln -s "../lib/$binaryName" "$binaryDirectory/$binaryName"
ln -s "../lib/$binaryName-bin" "$binaryDirectory/$binaryName-bin"
...
#replace "REPLACEME" with binaryName in the desktop file
sed -i "s|REPLACEME|/usr/lib/$binaryName/$binaryName|g" "$desktopFileDirectory/$binaryName.desktop"
...
rm "$appDirectory/precomplete"
rm "$appDirectory/update-settings.ini"
rm "$appDirectory/removed-files"
